...
db.serialize(() => {
  db.run('PRAGMA foreign_keys=ON', function () {
    db.run('Create Table if not exists students(id char(8), name char(20), password char(32), Primary Key(id))', function () {
      var nameList = readline('./db/name.txt')
      nameList.on('line', async function (data, linecount) {
        var [id, name] = data.split(' ')
        db.run('Insert Into students Values(?, ?, ?)', [id, name, null])
      })
    })
